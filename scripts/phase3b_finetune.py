...
def train_fine_tune():
    ...
    train_loader = DataLoader(combined_dataset, batch_size=16, shuffle=True, num_workers=0, collate_fn=collate_fn)  # Increased batch size for RTX 4090

    # Optimizer - only train segmentation head parameters
    optimizer = optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=1e-4, weight_decay=1e-4)

    # Training parameters
    num_epochs = 20  # Increased for RTX 4090
    ...
    # Update README
    update_readme_with_finetuning_results(checkpoint['epoch'], checkpoint['val_iou'])
...
### Training Results:
- **Best Epoch**: {epoch}
- **Best Validation IoU**: {val_iou:.4f}
- **Early Stopping Behavior**: Triggered after {epoch+1} epochs due to lack of improvement in validation IoU
...
"""
    ...
    if os.path.exists(readme_path):
        ...
        if "Phase 3B – Final Run (RTX 4090)" in content:
            # Replace existing section
            start_idx = content.find("## Phase 3B – Final Run (RTX 4090)")
            if start_idx != -1:
    ...
    print("Updated README.md with Phase 3B RTX 4090 results")

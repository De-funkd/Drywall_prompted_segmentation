"""
Visualization script for ensemble CLIPSeg model results.

This script creates visualizations comparing ground truth masks with predictions
from the ensemble model for both cracks and taping datasets.
...
def get_matching_files(predictions_dir, ground_truth_dir, images_dir):
    ...
    """
    # Get all prediction files (using threshold 0.5 files)
    pred_files = [f for f in os.listdir(predictions_dir) if f.endswith('_pred_mask_thr0.5.png')]
    ...
    for pred_file in pred_files:
        # Extract base name (remove '_pred_mask_thr0.5.png')
        base_name = pred_file.replace('_pred_mask_thr0.5.png', '')
...
def create_visualization(image_path, gt_mask_path, pred_mask_path, output_path, prompt, aggregation, iou, dice):
    """
...
def main():
    parser = argparse.ArgumentParser(description='Visualize ensemble CLIPSeg model results')
    parser.add_argument('--predictions-dir', type=str,
                        default='outputs/clipseg_ensemble',
    ...
    print("Starting visualization of ensemble CLIPSeg model results...")
    print("="*80)
    ...
    for dataset in datasets:
        ...
        for aggregation in aggregations:
            ...
            # Define prediction directory for this aggregation
            prediction_dir = os.path.join(args.predictions_dir, dataset['name'], aggregation)
            ...
            vis_output_dir = os.path.join(args.output_base, dataset['name'], aggregation, prompt_folder_name, "visualizations")
            pred_output_dir = os.path.join(args.output_base, dataset['name'], aggregation, prompt_folder_name, "predictions")
            ...
            if not matched_triplets:
                ...
                print(f"    Found {len(matched_triplets)} matching triplets in {dataset['name']}/{aggregation}")
            ...
            for pred_path, gt_paths, img_path in matched_triplets:
                ...
                base_name = pred_filename.replace('_pred_mask_thr0.5.png', '')
                ...
                if dataset['name'] == 'taping':
                    ...
                    create_visualization(img_path, best_gt_path, pred_path, vis_path, prompt, aggregation, iou, dice)
                    ...
                    vis_path = os.path.join(vis_output_dir, vis_filename)
                    create_visualization(img_path, gt_path, pred_path, vis_path, prompt, aggregation, iou, dice)
        ...
        dataset_count = sum(len(triplets) for agg in aggregations 
                           for triplets, _ in [get_matching_files(
                               os.path.join(args.predictions_dir, dataset['name'], agg),
                               gt_dir, image_dir
                           )])
        total_processed += dataset_count
        print(f"Completed {dataset['name']} dataset. Visualizations saved to {args.output_base}/{dataset['name']}/{aggregation}/{prompt_folder_name}/visualizations")
    
    print(f"\nVisualization completed! Total images processed: {total_processed}")
    print("Output structure created:")
    print("outputs/clipseg_ensemble/")
    print("├── cracks/")
    print("│   ├── mean/")
    print("│   │   └── drywall_crack/")
    print("│   │       ├── predictions/")
